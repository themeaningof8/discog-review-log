...
export const ADMIN_REVIEWS_MAIN_CLASS =
  "mx-auto max-w-5xl px-4 py-10 font-sans";

export type AdminReviewsPageContentProps = {
  items: {
    id: string;
    status: string;
    title: string;
...
export function AdminReviewsPageContent(props: AdminReviewsPageContentProps) {
  return (
    <>
          ...
          {props.items.map((it) => (
            <tr key={it.id} className="border-b border-neutral-900">
              ...
              <td className="py-3 text-neutral-500">
                {it.publishedAt ? formatCalendarDateTime(it.publishedAt) : "—"}
              </td>
            </tr>
          ))}
        </tbody>
